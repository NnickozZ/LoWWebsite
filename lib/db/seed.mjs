...
/**
 * §66: waaruit een stamboom getrokken kan worden.
 *
 * Personen en onderzoekers eerst, dan de abnormaliteit en het pantheon: een
 * bloedlijn blijft niet altijd menselijk, en wat een god verwekt is even goed
 * een tak. De volgorde is niet vrijblijvend — de "'X' aanmaken"-rij van de
 * artikelkiezer maakt een artikel van de *eerste* slug in `ofType`, dus die
 * hangt van de soort af waar het veld op staat (`afstammingVoor`).
 */
const AFSTAMMING = [
  'character',
  'investigator',
...
/**
 * §66: dezelfde zeven soorten, maar met de juiste voorop. Op de pagina van een
 * kosmische god is een onbekende ouder waarschijnlijk zelf een kosmische god;
 * op de pagina van een persoon is het een persoon. Alles behalve het pantheon
 * krijgt `character` vooraan.
 */
const afstammingVoor = (slug) =>
  PANTHEON.includes(slug)
    ? [slug, ...AFSTAMMING.filter((s) => s !== slug)]
    : ['character', ...AFSTAMMING.filter((s) => s !== 'character')];

/**
 * §66: de drie verwantschapsvelden die iedereen krijgt die in een stamboom kan
 * staan. `role` is wat een stamboom tekent en wat de server spiegelt (Ouder ↔
 * Kind, Partner ↔ Partner); het *label* is het woord op de lijn.
 */
const STAMBOOM_KERN = (slug) => [
  { key: 'ouders', label: 'Ouders', kind: 'entry_links', ofType: afstammingVoor(slug), role: 'parent' },
  { key: 'kinderen', label: 'Kinderen', kind: 'entry_links', ofType: afstammingVoor(slug), role: 'child' },
  { key: 'partner', label: 'Partner', kind: 'entry_links', ofType: afstammingVoor(slug), role: 'partner' },
  // §67 (ronde 33): broers en zussen leidt het archief zélf af uit gedeelde
  // ouders — dit veld is voor het geval dát niet kan: de ouders staan nergens,
  // of niemand weet wie ze waren, en iemand weet wél dat deze twee bij elkaar
  // horen. Rol `sibling`: dezelfde rij, geen verbintenis, en de server schrijft
  // de andere kant erbij zoals bij Partner.
  {
    key: 'broers_zussen',
    label: 'Broers en zussen',
    kind: 'entry_links',
    ofType: afstammingVoor(slug),
    role: 'sibling',
  },
];

/**
 * §66: en wat een god of een abnormaliteit erbij krijgt. Scheppen is ouderschap
 * met een ander woord erop, dus het is dezelfde rol met een eigen label; "Aspect
 * van" is `kin` — het wordt getekend en met opzet niet gespiegeld, want een
 * aspect is één kant van een ding en niet twee dingen naast elkaar.
 */
const STAMBOOM_GODEN = (slug) => [
  { key: 'geschapen_door', label: 'Geschapen door', kind: 'entry_links', ofType: afstammingVoor(slug), role: 'parent' },
  { key: 'schepselen', label: 'Schepselen', kind: 'entry_links', ofType: afstammingVoor(slug), role: 'child' },
  { key: 'aspect_van', label: 'Aspect van', kind: 'entry_links', ofType: afstammingVoor(slug), role: 'kin' },
];

/**
 * §67: er wás een Achternaam-veld (§66, ronde 31). Het is weg — de naam die van
 * een familie is staat al in het Familie-veld, dat de rand kleurt, en een tweede
 * plek voor dezelfde naam is een tweede plek die iets anders kan zeggen. Een
 * vers archief krijgt het niet meer; een bestaand archief raakt het kwijt via
 * `seed:round-33-drop-achternaam`, maar alleen als er niets in staat.
 */

/** §66: wie mensen zijn, en wie dat niet is. Twee lijsten, twee doosjes velden. */
const STAMBOOM_MENSEN = ['character', 'investigator'];
const STAMBOOM_MACHTEN = ['abnormality', ...PANTHEON];

/** §66: precies wat één soort erbij krijgt, in de volgorde waarin het aangeplakt wordt. */
const stamboomVelden = (slug) =>
  STAMBOOM_MENSEN.includes(slug)
    ? STAMBOOM_KERN(slug)
    : [...STAMBOOM_KERN(slug), ...STAMBOOM_GODEN(slug)];
...
export const ENTRY_TYPES = [
  {
    ...
    fields: [
      ...
      VEREERT_VELD,
      // §66: Ouders, Kinderen en Partner. In een vers archief staan ze hier; in
      // een bestaand archief plakt `seed:round-31-stamboom` ze er één keer bij,
      // zodat de twee hetzelfde zeggen. (§67: de Achternaam stond hier ook, en
      // is weg — zie `seed:round-33-drop-achternaam`.)
      ...stamboomVelden('character'),
    ],
  },
  {
    slug: 'investigator',
  ...
  },
  {
    ...
    fields: [
      ...
      /*
       * §79: en waar het in een kamer past.
       *
       * De kamer vraagt naar dít veld en niet naar deze soort, zodat er later
       * meer soorten in passen. Het staat hier omdat migratie 0027 op een leeg
       * archief vóór de seed draait en daar dus niets te wijzigen heeft; die
       * migratie doet hetzelfde voor een archief dat al bestaat. Leeg laten mag:
       * een voorwerp zonder plek is gewoon een voorwerp dat nergens op ligt.
       */
      {
        key: 'plek',
        label: 'Plek',
        // §83: meerkeuze, niet keuze. Een ding mag aan de muur én op de plank
        // passen; de kamer vraagt of de soort plek in de lijst staat.
        kind: 'multiselect',
        options: ['muur', 'plank', 'bureau', 'kist'],
      },
    ],
    // §80: een voorwerp is één ding in de wereld. Twee onderzoekers kunnen niet
    // allebei dezelfde lantaarn op hun plank hebben liggen.
    one_of_a_kind: 1,
  },
  {
    /*
     * §80: huisraad — het ding dat een kamer inricht.
     *
     * Niet hetzelfde als een voorwerp, en met opzet niet: een voorwerp wordt
     * gevónden (§24) en is uniek; huisraad wordt gekocht, heeft een prijs, en
     * meerdere onderzoekers mogen hetzelfde hebben. Alleen de Keeper maakt ze
     * (`keeper_made`) — dat is het hele verschil met de rest van de wiki.
     *
     * Het veld 'plek' draagt dezelfde sleutel als bij een voorwerp, want de
     * kamer vraagt naar het veld en niet naar de soort. Dat is de reden dat
     * huisraad erin past zonder één regel code in §79.
     */
    slug: 'huisraad',
    border: 'solid',
    label: 'Huisraad',
    ...
    keeper_made: 1,
    fields: [
      // §83: meerkeuze — hetzelfde stuk huisraad mag aan de muur én op een plank.
      { key: 'plek', label: 'Plek', kind: 'multiselect', options: ['muur', 'plank', 'bureau', 'kist'] },
      {
        key: 'prijs',
        label: 'Prijs',
        kind: 'number',
        hint: 'Leeg of 0: staat niet in de catalogus — die geef je zelf weg.',
      },
      {
        key: 'effect',
        label: 'Wat het geeft',
        kind: 'longtext',
        hint: 'Eén effect per regel. De kamer zet ze onder elkaar; het archief rekent er niets mee.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      DIENAAR_VELD,
      // §66: en het heeft een afkomst — Geschapen door, Schepselen, Aspect van.
      ...stamboomVelden('abnormality'),
    ],
  },
  /*
   ...
   */
  {
    ...
    fields: [
      ...
      // §66
      ...stamboomVelden('kosmische-goden'),
    ],
  },
  {
    slug: 'aardse-goden',
  ...
  },
  {
    ...
    fields: [
      ...
      // §66
      ...stamboomVelden('eldritch-entiteiten'),
    ],
  },
  {
    ...
    fields: [
      ...
      // §66
      ...stamboomVelden('bovennatuurlijke-wezens'),
    ],
  },
  {
    slug: 'faction',
  ...
  },
  {
    ...
    fields: [
      ...
      // §66 (ronde 32): de stamboom van deze familie. Niet elke stamboom hoort
      // bij een familie — maar een familie moet er een kunnen aanwijzen, en dit
      // is de enige soort die het veld geseed krijgt. Leeg laten mag altijd.
      // In een bestaand archief plakt `seed:round-32-stamboom-link` het erbij.
  ...
  },
  {
    ...
    fields: [
      ...
      // Ronde 30: "Gespeeld op" was het enige veld op een sessierapport dat
      // over de tafel ging in plaats van over de wereld. Een rapport in dit
      // archief is een stuk papier met een datum erop, en zo'n datum heet
      // hier een verslagdatum.
      { key: 'date_played', label: 'Verslagdatum', kind: 'date' },
...
export function seedBaseline(sqlite) {
  ...
  for (const t of ENTRY_TYPES) {
    ...
    insertType.run(
      ...
      t.sort_order,
      t.case_only ?? 0,
      // §80: twee vlaggen op een soort. Ze staan hier én in migratie 0028 — de
      // migratie draait op een leeg archief vóór deze seed en heeft daar niets
      // te vinden, en op een archief dat al bestaat is het andersom.
      t.keeper_made ?? 0,
      t.one_of_a_kind ?? 0,
    );
  ...
  // Ronde 30: "Gespeeld op" wordt "Verslagdatum" op het sessierapport. Eén keer,
  // en alleen waar het label nog woord voor woord het geleverde is — een Keeper
  // die dat veld zelf al anders genoemd heeft houdt zijn woord (§11). De sleutel
  // `date_played` verandert niet: daar hangen de ingevulde datums aan, en een
  // hernoemde sleutel zou ze allemaal laten vallen.
  const verslagdatumMarker = 'seed:round-30-verslagdatum';
  const verslagdatumDone = sqlite
    .prepare('SELECT name FROM schema_migrations WHERE name = ?')
    .get(verslagdatumMarker);
  if (!verslagdatumDone) {
    const row = sqlite.prepare("SELECT fields FROM entry_types WHERE slug = 'session'").get();
    if (row?.fields) {
      try {
        ...
        let touched = false;
        for (const field of Array.isArray(fields) ? fields : []) {
          if (field?.key === 'date_played' && field.label === 'Gespeeld op') {
            field.label = 'Verslagdatum';
            touched = true;
          }
        }
        if (touched) {
    ...
    sqlite.prepare('INSERT OR IGNORE INTO schema_migrations (name) VALUES (?)').run(verslagdatumMarker);
  }

  // §66: de stamboom. Dezelfde vorm als Families (§51), Talen (§55) en het
  // pantheon (§58), en om dezelfde reden: in een vers archief staan Ouders,
  // Kinderen, Partner en de rest gewoon in `ENTRY_TYPES`, maar de zeven soorten
  // die er al zijn moeten ze één keer met de hand aangeplakt krijgen — anders
  // zegt een opgewaardeerd archief iets anders dan een nieuw. Aanplakken, nooit
  // overschrijven: een soort die al een veld met die sleutel heeft houdt het,
  // hoe de Keeper het sindsdien ook genoemd, gericht of van rol veranderd heeft,
  // en een soort die al aan de twintig velden zit wordt overgeslagen.
  const stamboomMarker = 'seed:round-31-stamboom';
  const stamboomDone = sqlite
    .prepare('SELECT name FROM schema_migrations WHERE name = ?')
    .get(stamboomMarker);
  if (!stamboomDone) {
    const setFields = sqlite.prepare('UPDATE entry_types SET fields = ? WHERE slug = ?');
    for (const slug of [...STAMBOOM_MENSEN, ...STAMBOOM_MACHTEN]) {
      const row = sqlite.prepare('SELECT fields FROM entry_types WHERE slug = ?').get(slug);
      if (!row) continue;
      try {
        const fields = JSON.parse(row.fields || '[]');
        if (!Array.isArray(fields)) continue;
        let geraakt = false;
        for (const veld of stamboomVelden(slug)) {
          if (fields.some((field) => field?.key === veld.key)) continue;
          if (fields.length >= 20) break;
          fields.push(veld);
          geraakt = true;
    ...
    sqlite.prepare('INSERT OR IGNORE INTO schema_migrations (name) VALUES (?)').run(stamboomMarker);
  }

  // §66 (ronde 32): "Stambomen moeten gelinkt kunnen worden aan families (niet
  // altijd, maar families moet een link hebben hiervoor)." Eén veld op één
  // soort, in dezelfde vorm als hierboven: aanplakken, een bestaande sleutel
  // overslaan, stoppen bij twintig velden, en een handgezette rij met rust
  // laten. Alleen Families krijgt het — een stamboom hoort bij een familie, en
  // een familie heeft er hooguit één.
  const stamboomLinkMarker = 'seed:round-32-stamboom-link';
  const stamboomLinkDone = sqlite
    .prepare('SELECT name FROM schema_migrations WHERE name = ?')
    .get(stamboomLinkMarker);
  if (!stamboomLinkDone) {
    const row = sqlite.prepare("SELECT fields FROM entry_types WHERE slug = 'family'").get();
    if (row) {
      try {
        const fields = JSON.parse(row.fields || '[]');
        if (Array.isArray(fields) && fields.length < 20 && !fields.some((field) => field?.key === 'stamboom')) {
          fields.push({ key: 'stamboom', label: 'Stamboom', kind: 'family_tree_link' });
          sqlite
    ...
    sqlite.prepare('INSERT OR IGNORE INTO schema_migrations (name) VALUES (?)').run(stamboomLinkMarker);
  }

  /*
   * §67 (ronde 33): Broers en zussen. Precies de vorm van §51/§55/§58/§66
   * hierboven, en om precies dezelfde reden: in een vers archief staat het veld
   * gewoon in `ENTRY_TYPES` (`STAMBOOM_KERN`), maar de zeven soorten die er al
   * zijn hebben `seed:round-31-stamboom` al gehad en zouden het nooit meer
   * krijgen. Aanplakken, nooit overschrijven: een soort die de sleutel al heeft
   * houdt hem — hoe de Keeper hem sindsdien ook genoemd of van rol veranderd
   * heeft — en een soort die al aan de twintig velden zit wordt overgeslagen.
   *
   * Het veld is met opzet niet het gewone geval: broers en zussen leidt het
   * archief zélf af uit gedeelde ouders (`lib/families/siblings.ts`). Dit is
   * het doosje voor wat je niet kunt afleiden.
   */
  const broersZussenMarker = 'seed:round-33-broers-zussen';
  const broersZussenDone = sqlite
    .prepare('SELECT name FROM schema_migrations WHERE name = ?')
    .get(broersZussenMarker);
  if (!broersZussenDone) {
    const setFields = sqlite.prepare('UPDATE entry_types SET fields = ? WHERE slug = ?');
    for (const slug of [...STAMBOOM_MENSEN, ...STAMBOOM_MACHTEN]) {
      const row = sqlite.prepare('SELECT fields FROM entry_types WHERE slug = ?').get(slug);
      ...
      try {
        const fields = JSON.parse(row.fields || '[]');
        if (!Array.isArray(fields)) continue;
        if (fields.some((field) => field?.key === 'broers_zussen')) continue;
        if (fields.length >= 20) continue;
        fields.push({
          key: 'broers_zussen',
          label: 'Broers en zussen',
        ...
        setFields.run(JSON.stringify(fields), slug);
      } catch {
        /* a hand-edited row is the Keeper's, and stays theirs */
      }
    }
    sqlite.prepare('INSERT OR IGNORE INTO schema_migrations (name) VALUES (?)').run(broersZussenMarker);
  }

  /*
   * §67 (ronde 33): de Achternaam gaat er weer af, en dit keert ronde 31 om.
   *
   * Dezelfde markervorm als hierboven, maar de andere kant op — en dus met een
   * regel die de andere blokken niet nodig hebben: **een veld waar iets in
   * staat blijft staan.** Een archief dat ronde 31 heeft meegemaakt kan het
   * doosje ingevuld hebben, en dat is de Keeper z'n tekst, niet de onze. Dus
   * per soort één telling: staat er ergens een niet-lege `achternaam` op een
   * artikel dat niet in de prullenbak ligt, dan blijft de velddefinitie waar
   * hij is en zegt niemand er iets over. Staat er nergens iets in, dan gaat de
   * lege doos weg.
   *
   * En het is een *nauwe* match: sleutel `achternaam` én kind `text`. Een
   * Keeper die die sleutel hergebruikt heeft voor iets anders — een
   * koppelingsveld, een keuzelijst — houdt zijn veld, want dan is het zijn veld
   * en niet meer het onze (§11).
   *
   * De marker valt hoe dan ook, ook als beide soorten hun veld houden: dit is
   * een eenmalige opruiming, geen regel die elke start opnieuw langsloopt.
   */
  const dropAchternaamMarker = 'seed:round-33-drop-achternaam';
  const dropAchternaamDone = sqlite
    .prepare('SELECT name FROM schema_migrations WHERE name = ?')
    .get(dropAchternaamMarker);
  if (!dropAchternaamDone) {
    // Eén telling per soort. `json_valid` houdt `json_extract` weg van een rij
    // die met de hand gezet is, en TRIM(COALESCE(…)) leest een spatie als leeg.
    const gevuld = sqlite.prepare(
      `SELECT COUNT(*) AS n
         FROM entries e
         JOIN entry_types t ON t.id = e.type_id
        WHERE t.slug = ?
          AND e.deleted_at IS NULL
          AND json_valid(e.fields)
          AND TRIM(COALESCE(json_extract(e.fields, '$.achternaam'), '')) <> ''`,
    );
    const setFields = sqlite.prepare('UPDATE entry_types SET fields = ? WHERE slug = ?');
    for (const slug of STAMBOOM_MENSEN) {
      const row = sqlite.prepare('SELECT fields FROM entry_types WHERE slug = ?').get(slug);
      if (!row) continue;
      try {
        const fields = JSON.parse(row.fields || '[]');
        if (!Array.isArray(fields)) continue;
        if (!fields.some((field) => field?.key === 'achternaam' && field?.kind === 'text')) continue;
        if ((gevuld.get(slug)?.n ?? 0) > 0) continue;
        setFields.run(
          JSON.stringify(fields.filter((field) => !(field?.key === 'achternaam' && field?.kind === 'text'))),
          slug,
        );
      } catch {
        /* a hand-edited row is the Keeper's, and stays theirs */
    ...
    sqlite.prepare('INSERT OR IGNORE INTO schema_migrations (name) VALUES (?)').run(dropAchternaamMarker);
  }
    ...
    sqlite
      .prepare(
        `INSERT INTO site_settings (id, name, tagline, invite_code, theme)
         VALUES (1, 'LoW: Land over Water Archief', 'Archief van het Eiland', ?, '{}')`,
      )
      .run(makeInviteCode());
  }
}
