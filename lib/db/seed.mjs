...
export const ENTRY_TYPES = [
  ...
  },
  {
    ...
    sort_order: 90,
    fields: [
      // §55: een overlevering is overgeleverd *in* iets — het rijmpje, de
      // grafsteen, het gezang. Het enige veld dat deze soort ooit gekregen heeft.
      { key: 'talen', label: 'Talen', kind: 'entry_links', ofType: ['language'] },
    ],
  },
  {
...
const EXAMPLE_BLOCKS = {
  ...
  language: [
    ...
    { id: 'body', kind: 'body' },
    {
      ...
      open: true,
      fromType: ['character', 'investigator', 'object', 'abnormality', 'faction', 'lore'],
      viaField: 'talen',
      sort: 'name',
    },
    { id: 'sections', kind: 'sections' },
...
export function seedBaseline(sqlite) {
  ...
  if (!talenDone) {
    ...
    const rows = sqlite
      .prepare(
        "SELECT slug, fields FROM entry_types WHERE slug IN ('character', 'investigator', 'object', 'abnormality', 'faction', 'lore')",
      )
      .all();
    for (const row of rows) {
      try {
