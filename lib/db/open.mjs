...
function applyMigrations(sqlite) {
  ...
  for (const migration of MIGRATIONS) {
    ...
    try {
      ...
      // §95: a migration may carry a step in JavaScript as well — for a
      // conversion SQL cannot say (0034 reads every short text with the
      // reader's own algorithm). Same transaction, same all-or-nothing.
      if (typeof migration.run === 'function') migration.run(sqlite);
