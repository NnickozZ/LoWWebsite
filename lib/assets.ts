...
import { PLAYER_UPLOAD_BYTES, tooLargeMessage } from '@/lib/upload';

/**
 * How much one upload may weigh: 2 MB for a player, 20 MB for a Keeper
 * (Nick, 6 Sep 2026). The numbers and the three functions around them moved
 * to `lib/upload.ts`, which is pure — this file opens the database and loads
 * sharp, so a client component could never import from it, and the browser
 * now needs the ceiling too: an oversized player's picture is shrunk to fit
 * before it is sent rather than refused. They are re-exported here because
 * this is where the server has always read them, and because the ceiling is
 * *enforced* here and nowhere else: checked twice, on the size the browser
 * declared before the body is read, and again on the bytes that actually
 * arrived, so neither a mislabelled file nor a browser that skipped the
 * shrinking can slip past.
 *
 * Two things sit outside the app: a reverse proxy in front of the server
 * (nginx's `client_max_body_size` defaults to 1 MB and must be raised to at
 * least 25m), and sharp's own pixel ceiling (`limitInputPixels`, ~268 MP by
 * default), which is what stops a big scan from eating the box's memory.
 */
export {
  PLAYER_UPLOAD_BYTES,
  KEEPER_UPLOAD_BYTES,
